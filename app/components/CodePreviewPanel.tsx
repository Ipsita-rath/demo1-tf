...
export default function CodePreviewPanel({ resources, globalConfig, activeTab, onTabChange, onClose }: CodePreviewPanelProps) {
  ...
  const { data: generatedCode, isLoading: isGenerating, error } = useQuery({
    queryKey: ['/api/terraform/generate-code', resources, globalConfig, JSON.stringify(globalConfig?.adRoles || [])],
    queryFn: async () => {
      if (!resources || resources.length === 0) return { code: '' };
      
      console.log('CodePreviewPanel - Generating code for resources:', resources);
      console.log('CodePreviewPanel - Using global config:', globalConfig);
      console.log('CodePreviewPanel - AD roles in config:', globalConfig?.adRoles);
      ...
      try {
       const result = await generateTerraformCode(      //updated it for filterationin code for roles
        resources,
        terraformToken || undefined,
        usePrivateModules,
      {
          ...globalConfig,
          landingZoneId: globalConfig?.landingZoneId || localStorage.getItem('selectedLandingZoneId'),
      }
      );
          ...
          <TabsContent value="code" className="p-4 h-full overflow-y-auto bg-gray-50 dark:bg-gray-800">
            ...
            {isGenerating ? (
              ...
              <div className="flex flex-col items-center justify-center h-32 text-center">
                ...
                <div className="text-red-600 dark:text-red-400">Failed to generate Terraform code</div>
                ...
                <CardContent className="p-4">
                  ...
                  {deploymentStatus === 'ready' && (
                    <Button 
                      ...
                      disabled={!generatedCode?.code || !resources || resources.length === 0}
                    >
