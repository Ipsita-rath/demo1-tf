...
interface ConfigurationPanelProps {
  resource?: TerraformResource;  // Make optional since it may be null for role assignment
...
export default function ConfigurationPanel({
  ...
  if (configurationMode === 'role_assignment' && selectedResourceGroup) {
    return renderRoleAssignmentForm();
  }
  ...
  const handleNameChange = (newName: string) => {
    setName(newName);
    const validation = validateAzureName(newName, resource.type);
    setNameValidation(validation);
  };
  ...
  const namingRule = getResourceNamingRule(resource.type);
  ...
  const handleApplyConfiguration = () => {
    ...
    console.log('ConfigurationPanel (client): Final updated resource:', updatedResource);
    console.log('ConfigurationPanel (client): config.name is now:', updatedResource.config.name);
  ...
  // AD Roles Configuration Component
...
const renderRoleAssignmentForm = () => {
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Group Email</label>
          <input
            type="email"
        ...
        <div>
          ...
          <div className="flex gap-4">
            {['Owner', 'Contributor', 'Reader'].map((role) => (
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          <Switch
            id="enableRoleAssignments"
            checked={config.enableRoleAssignments || false}
            onCheckedChange={(checked) => handleConfigChange('enableRoleAssignments', checked)}
          />
          <Label htmlFor="enableRoleAssignments">Enable Azure AD Role Assignments</Label>
        </div>
        
        {config.enableRoleAssignments && (
          <div className="pl-4 space-y-3">
            <div className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Azure AD Roles
            </div>
            <div className="space-y-2">
              {(config.adRoles || []).map((role: string, index: number) => (
                <div key={index} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800 rounded">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      const newRoles = [...(config.adRoles || [])];
                      newRoles.splice(index, 1);
                      handleConfigChange('adRoles', newRoles);
                    }}
                  >
            ...
            <div className="flex space-x-2">
              <Select 
                value="" 
                onValueChange={(value) => {
                  if (value && !config.adRoles?.includes(value)) {
                    handleConfigChange('adRoles', [...(config.adRoles || []), value]);
                  }
                }}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Key Vault Administrator">Key Vault Administrator</SelectItem>
                  <SelectItem value="Key Vault Secrets Officer">Key Vault Secrets Officer</SelectItem>
                  <SelectItem value="Storage Blob Data Owner">Storage Blob Data Owner</SelectItem>
                  <SelectItem value="Storage Blob Data Contributor">Storage Blob Data Contributor</SelectItem>
                  <SelectItem value="Virtual Machine Contributor">Virtual Machine Contributor</SelectItem>
                  <SelectItem value="Network Contributor">Network Contributor</SelectItem>
                  <SelectItem value="SQL DB Contributor">SQL DB Contributor</SelectItem>
                  <SelectItem value="SQL Server Contributor">SQL Server Contributor</SelectItem>
                  <SelectItem value="AI Developer">AI Developer</SelectItem>
                  <SelectItem value="API Management Service Contributor">API Management Service Contributor</SelectItem>
                  <SelectItem value="Managed Identity Contributor">Managed Identity Contributor</SelectItem>
                  <SelectItem value="Cognitive Services OpenAI Contributor">Cognitive Services OpenAI Contributor</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="text-xs text-gray-500 mt-2">
              Role assignments will be automatically created for the current user with selected roles.
            </div>
          </div>
        )}
      </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  const validation = validateAzureName(value, 'key_vault');
                  setNameValidation(validation);
                }}
  ...
  return (
    <div className="w-full h-full bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 shadow-sm flex flex-col">
      ...
      {/* Configuration Form */}
      <div className="flex-1 overflow-y-auto" style={{minHeight: 0, maxHeight: 'calc(100vh - 250px)'}}>
        <div className="p-4 space-y-4" style={{paddingBottom: '4rem'}}>
          <div className="w-full">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              ...
              {namingRule.additionalRules && namingRule.additionalRules.slice(0, 2).map((rule, index) => (
